import { Injectable, Directive, OnInit, forwardRef } from '@angular/core';
import { NgControl, NG_VALIDATORS, ValidationErrors, Validator, AbstractControl } from '@angular/forms';
...
export class ValidacaoService implements Validator {
  ...
  validate(control: AbstractControl): ValidationErrors | null {
    if (control && control.value) {
      if (control.value.length < 10) return { 'phone-mask-br': true };
    }
    return null;
  }
}
...
export class phoneMaskBrDirective implements OnInit {
  ...
  private _setVal(val: string) {
    if (this.ngControl.control) this.ngControl.control.setValue(val, { emitEvent: false });
  }

  private _maskValue(val: any): string {
    if (val) {
      let newVal: string = val;
      if (!isNaN(val)) newVal = val.toString();
  ...
  private _unmaskValue(val: string): string {
    ...
    if (maskedVal !== val) this._setVal(maskedVal);
